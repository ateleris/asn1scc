...
from conftest import get_null_terminator_string, generate_test_string, \
    generate_ia5_string, generate_ia5_string_random_length
...
def _encode_and_decode_single_ia5_string(acn_encoder: ACNEncoder, input_string: str, max_length: int) -> tuple[bool, str]:
    ...
    """
    encoded_res = acn_encoder.enc_ia5_string_char_index_external_field_determinant(max_length, input_string)
    if not encoded_res.success:
        return False, ""

    acn_decoder: ACNDecoder = acn_encoder.get_decoder()
    decoded_res = acn_decoder.dec_ia5_string_char_index_external_field_determinant(max_length, len(input_string))
...
def _encode_and_decode_multiple_ia5_strings(acn_encoder: ACNEncoder, input_strings: list[str], max_length: int) -> tuple[bool, list[str]]:
    ...
    for input_number in input_strings:
        encoded_res = acn_encoder.enc_ia5_string_char_index_external_field_determinant(max_length, input_number)
        if not encoded_res.success:
            return False, []
    ...
    for _ in range(len(input_strings)):
        decoded_res = acn_decoder.dec_ia5_string_char_index_external_field_determinant(max_length, max_length)
        if not decoded_res.success:
            return False, []
...
def _test_single_string(acn_encoder: ACNEncoder, input_string: str, max_length: int) -> None:
    ...
    success, decoded_value = _encode_and_decode_single_ia5_string(acn_encoder, input_string, max_length)
...
def _test_multiple_strings(acn_encoder: ACNEncoder, input_numbers: list[str], max_length: int) -> None:
    ...
    success, decoded_values = _encode_and_decode_multiple_ia5_strings(acn_encoder, input_numbers, max_length)
...
def test_enc_dec_ia5_string_char_index_external_field_determinant_single_value(acn_encoder: ACNEncoder, seed: int, max_length: int) -> None:
    input_string: str = generate_ia5_string(max_length)
    _test_single_string(acn_encoder, input_string, max_length)

def test_enc_dec_ia5_string_char_index_external_field_determinant_single_value_var_length(acn_encoder: ACNEncoder, seed: int, max_length: int) -> None:
...
def test_enc_dec_ia5_string_char_index_external_field_determinant_multiple_values(acn_encoder: ACNEncoder, seed: int, max_length: int) -> None:
    input_strings: list[str] = []
    for i in range(random.randint(3, 10)):
        input_strings.append(generate_ia5_string(max_length))

    _test_multiple_strings(acn_encoder, input_strings, max_length)

def test_enc_dec_ia5_string_char_index_external_field_determinant_zero_length(acn_encoder: ACNEncoder, seed: int) -> None:
    input_string: str = ""
    encoded_res = acn_encoder.enc_ia5_string_char_index_external_field_determinant(0, input_string)
    assert encoded_res.success
    with pytest.raises(AssertionError) as excinfo:
        acn_decoder: ACNDecoder = acn_encoder.get_decoder()
    assert "Codec buffer_size must be greater than zero" in str(excinfo.value)

def test_enc_dec_ia5_string_char_index_external_field_determinant_null_terminator_symbol(acn_encoder: ACNEncoder, seed: int, max_length: int) -> None:
    input_string: str = get_null_terminator_string(max_length)
    encoded_res = acn_encoder.enc_ia5_string_char_index_external_field_determinant(0, input_string)
    assert encoded_res.success, f"Encoding/decoding did not fail for input {input_string}"
    assert encoded_res.bits_encoded == 0

def test_enc_dec_ia5_string_char_index_external_field_determinant_too_long(acn_encoder: ACNEncoder, seed: int, max_length: int, charset: str) -> None:
