...
def _encode_and_decode_single_string(acn_encoder: ACNEncoder, input_string: str, max_length: int) -> tuple[bool, str]:
    ...
    """
    encoded_res = acn_encoder.enc_string_ascii_fix_size(max_length, input_string)
...
def _encode_and_decode_multiple_strings(acn_encoder: ACNEncoder, input_strings: list[str], max_length: int) -> tuple[bool, list[str]]:
    ...
    for input_number in input_strings:
        encoded_res = acn_encoder.enc_string_ascii_fix_size(max_length, input_number)
        if not encoded_res.success:
            return False, []
...
def test_enc_dec_string_ascii_fix_size_zero_length(acn_encoder: ACNEncoder, seed: int) -> None:
    input_string: str = ""
    encoded_res =  encoded_res = acn_encoder.enc_string_ascii_fix_size(0, input_string)
