...
from conftest import get_max_uint_ascii, get_min_uint_ascii, get_random_uint_ascii
...
def _encode_and_decode_unsigned_integer_as_ascii(acn_encoder: ACNEncoder, input_number: int, byte: int) -> tuple[
    ...
    """
    encoded_res = acn_encoder.enc_uint_ascii_const_size(input_number, byte)
    if not encoded_res.success:
        return False, 0

    acn_decoder: ACNDecoder = acn_encoder.get_decoder()
    decoded_res = acn_decoder.dec_uint_ascii_const_size(byte)
...
def _encode_and_decode_multiple_unsigned_integers_as_ascii(acn_encoder: ACNEncoder, input_numbers: list[int], byte: int) -> tuple[
    bool, list[int]]:
...
def test_enc_dec_uint_ascii_const_size_single_value(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
    ...
    _test_single_unsigned_integer(acn_encoder, input_number, byte)
...
def test_enc_dec_uint_ascii_const_size_multiple_values(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
    ...
    _test_multiple_unsigned_integers(acn_encoder, input_numbers, byte)

def test_enc_dec_uint_ascii_const_size_zero(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
    input_number: int = 0
    _test_single_unsigned_integer(acn_encoder, input_number, byte)
