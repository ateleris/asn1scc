...
def _encode_and_decode_single_string(acn_encoder: ACNEncoder, input_string: str, max_length: int, charset: str) -> tuple[bool, str]:
    ...
    """
    encoded_res = acn_encoder.enc_string_char_index_fix_size(max_length, charset_to_bytes(charset), input_string)
    if not encoded_res.success:
        return False, ""

    acn_decoder: ACNDecoder = acn_encoder.get_decoder()
    decoded_res = acn_decoder.dec_string_char_index_fix_size(max_length, charset_to_bytes(charset))
...
def _encode_and_decode_multiple_strings(acn_encoder: ACNEncoder, input_strings: list[str], max_length: int, charset: str) -> tuple[bool, list[str]]:
    ...
    for input_number in input_strings:
        encoded_res = acn_encoder.enc_string_char_index_fix_size(max_length, charset_to_bytes(charset), input_number)
        if not encoded_res.success:
            return False, []
...
def _test_single_string_starts_with(acn_encoder: ACNEncoder, input_string: str, max_length: int, charset: str) -> None:
    """Helper function to test encoding/decoding of a single string, where de decoded value starts with the input_string provided."""
    success, decoded_value = _encode_and_decode_single_string(acn_encoder, input_string, max_length, charset)
    assert success, f"Encoding/decoding failed for input {input_string}"
    print(f"Input: {input_string}, decoded {decoded_value}, Passed: {input_string == decoded_value}")
...
def test_enc_dec_string_char_index_fix_size_single_value(acn_encoder: ACNEncoder, seed: int, max_length: int, charset: str) -> None:
    input_string: str = generate_test_string(charset, max_length)
    _test_single_string(acn_encoder, input_string, max_length, charset)

def test_enc_dec_string_char_index_fix_size_single_value_var_length(acn_encoder: ACNEncoder, seed: int, max_length: int, charset: str) -> None:
...
def test_enc_dec_string_char_index_fix_size_multiple_values(acn_encoder: ACNEncoder, seed: int, max_length: int, charset: str) -> None:
...
def test_enc_dec_string_char_index_fix_size_zero_length(acn_encoder: ACNEncoder, seed: int, charset: str) -> None:
    input_string: str = ""
    encoded_res = acn_encoder.enc_string_char_index_fix_size(0, charset_to_bytes(charset), input_string)
    assert encoded_res.success
    with pytest.raises(AssertionError) as excinfo:
        acn_decoder: ACNDecoder = acn_encoder.get_decoder()
    assert "Codec buffer_size must be greater than zero" in str(excinfo.value)

def test_enc_dec_string_char_index_fix_size_null_terminator_symbol(acn_encoder: ACNEncoder, seed: int, max_length: int, charset: str) -> None:
    input_string: str = get_null_terminator_string(max_length)
    success, decoded_value = _encode_and_decode_single_string(acn_encoder, input_string, max_length, charset)
    assert not success, f"Encoding/decoding did not fail for input {input_string} with charset {charset}"

def test_enc_dec_string_char_index_fix_size_too_long(acn_encoder: ACNEncoder, seed: int, max_length: int, charset: str) -> None:
