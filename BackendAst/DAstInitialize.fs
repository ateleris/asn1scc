...
let createIntegerInitFunc (r:Asn1AcnAst.AstRoot) (lm:LanguageMacros)  (t:Asn1AcnAst.Asn1Type) (o:Asn1AcnAst.Integer) (typeDefinition:TypeDefinitionOrReference)  =
    ...
    let sType = typeDefinition.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName
    // let sType = sType + "WHATTHEACTUALFROG"
...
let createNullTypeInitFunc (r:Asn1AcnAst.AstRoot)  (lm:LanguageMacros) (t:Asn1AcnAst.Asn1Type) (o :Asn1AcnAst.NullType) (typeDefinition:TypeDefinitionOrReference)  =
    ...
    let funcBody (p:CodegenScope) v =
        let resVar = p.accessPath.asIdentifier
        initNull (lm.lg.getValue p.accessPath) p.accessPath.isOptional resVar (typeDefinition.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName)
    let constantInitExpression () = "0"
    let testCaseFuncs: AutomaticTestCase list =
        [{AutomaticTestCase.initTestCaseFunc =
            (fun p ->
                let resVar = p.accessPath.asIdentifier
                {InitFunctionResult.funcBody = initNull (lm.lg.getValueUnchecked p.accessPath PartialAccess) p.accessPath.isOptional resVar (typeDefinition.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName); resultVar = resVar; localVariables=[]});
...
let createBooleanInitFunc (r:Asn1AcnAst.AstRoot) (lm:LanguageMacros) (t:Asn1AcnAst.Asn1Type) (o :Asn1AcnAst.Boolean) (typeDefinition:TypeDefinitionOrReference)  =
    ...
    let funcBody (p:CodegenScope) (v:Asn1ValueKind) =
        ...
        let vl =
            ...
            | BooleanValue iv   -> iv
            | _                 -> raise(BugErrorException "UnexpectedValue")
        initBoolean (lm.lg.getValue p.accessPath) vl p.accessPath.isOptional resVar (typeDefinition.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName)

    let initTestCaseFunc (vl: bool) (p: CodegenScope) =
        let resVar = p.accessPath.asIdentifier
        {InitFunctionResult.funcBody = initBoolean (lm.lg.getValueUnchecked p.accessPath PartialAccess) vl p.accessPath.isOptional resVar (typeDefinition.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName); resultVar = resVar; localVariables = []}
    ...
    let tasInitFunc (p:CodegenScope)  =
        ...
        | true    -> {InitFunctionResult.funcBody = initBoolean (lm.lg.getValue p.accessPath) false p.accessPath.isOptional resVar (typeDefinition.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName); resultVar = resVar; localVariables = []}
        | false     -> {InitFunctionResult.funcBody = initBoolean (lm.lg.getValue p.accessPath) true p.accessPath.isOptional resVar (typeDefinition.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName); resultVar = resVar; localVariables = []}
        ...
        List.map (fun vl ->
            {
                AutomaticTestCase.initTestCaseFunc =
                    (fun (p:CodegenScope) ->
                        let resVar = p.accessPath.asIdentifier
                        {InitFunctionResult.funcBody = initEnumerated (lm.lg.getValue p.accessPath) (lm.lg.getNamedItemBackendName (Some typeDefinition) vl) (typeDefinition.longTypedefName2  (Some lm.lg) (if ProgrammingLanguage.ActiveLanguages.Head = Python then false else lm.lg.hasModules) t.moduleName) p.accessPath.isOptional resVar; resultVar = resVar; localVariables=[]});
                        ...
                        let testCaseFunc (p: CodegenScope): InitFunctionResult =
                            ...
                            let bodyRes =
                                if lm.lg.decodingKind = Copy then
                                    let seqBuild = lm.uper.sequence_build resVar (typeDefinition.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName) p.accessPath.isOptional (children |> List.map (fun ch -> ch.resultVar))
                                    joinedBodies + "\n" + seqBuild
                                else joinedBodies
                ...
                | None  ->
                    ...
                    | None  ->
                        ...
                        | ReferenceToExistingDefinition    rf   when (not rf.definedInRtl) ->
                            let fncName = (ch.Type.typeDefinitionOrReference.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName) + (lm.init.methodNameSuffix())
        ...
        let childrenOut =
            ...
            | ChValue iv     ->
                ...
                List.choose(fun chChild ->
                    ...
                    | false -> None
                    | true  ->
                        let sChildTypeName = chChild.chType.typeDefinitionOrReference.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName
                        let sChoiceTypeName = typeDefinition.longTypedefName2 (Some lm.lg) lm.lg.hasModules t.moduleName
